...
const bannerMobileManager = () => {
    let i = Math.floor(Math.random() * 6);
    document.querySelector('#bannerMobile').style.backgroundImage = mobileBanners[i];
};
...
const stickyManager = () => {
    ...
    } else if ((window.pageYOffset + 86) > 591 && (window.pageYOffset + 85) < 1075) {
...
const counterController = () => {
    if (document.body.classList == "home-page") {
        ...
        let x = setInterval(() => {
            ...
            if (distance < 0) {
                clearInterval(x);
                document.querySelector('.counterTitle').innerText = `It's Mardi Gras!!!`;
